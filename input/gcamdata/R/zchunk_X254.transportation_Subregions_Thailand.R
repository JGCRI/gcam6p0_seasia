...
module_energy_X254.transportation_Subregions_Thailand <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    return(c("X201.Pop_Subregions_Thailand",
             ...
             FILE = "breakout/Subregions_Thailand_trn_shares",
    ...
    Subregions_Thailand_trn_shares <- get_data(all_data, "breakout/Subregions_Thailand_trn_shares")
    ...
    trn_shares <- Subregions_Thailand_trn_shares %>%
      #filter(!grepl("International|Aviation", tranSubsector)) %>%
      mutate(year = min(MODEL_BASE_YEARS)) %>%
      rbind(Subregions_Thailand_trn_shares %>% mutate(year = max(MODEL_BASE_YEARS))) %>%
      ...
      left_join_error_no_match(X254.pop_gdp_share_Subregions_Thailand,
                               by = c("region", "year")) %>%
    ...
    X254.StubTechProd_nonmotor <- L254.StubTechProd_nonmotor %>%
      ...
      downscale_to_breakout_regions(data = .,
                                    composite_region = "Thailand",
                                    disag_regions = c(subregions),
                                    share_data = X254.pop_gdp_share_Subregions_Thailand,
                                    value.column = "calOutputValue",
                                    share.column = "popshare",
    ...
    X254.StubTranTechCalInput_trn_Subregions_Thailand %>%
      ...
      left_join(X254.list_nochange_data_Subregions_Thailand[["L254.StubTranTechLoadFactor"]],
                by = c("region", "supplysector", "tranSubsector",
                       "stub.technology", "year", "sce")) %>%
      left_join(X254.list_nochange_data_Subregions_Thailand[["L254.StubTranTechCoef"]],
                by = c("region", "supplysector", "tranSubsector",
      ...
      L254.StubTranTechOutput_Subregions_Thailand
    ...
    L254.StubTranTechOutput_Subregions_Thailand %>%
      ...
      X254.BaseService_trn_Subregions_Thailand # OUTPUT
    ...
    X254.StubTechCalInput_passthru_trn_Subregions_Thailand <- L254.StubTranTechOutput_Subregions_Thailand %>%
      mutate(
