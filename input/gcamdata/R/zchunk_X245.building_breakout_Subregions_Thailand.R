...
#' \code{X245.HDDCDD_AEO_2015_bld_Subregions_Thailand}, \code{X245.HDDCDD_constdds_bld_Subregions_Thailand}, \code{X245.GompFnParam_Thailand}.
...
module_gcamseasia_X245.building_breakout_Subregions_Thailand <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    X245.DeleteSupplysector_bld_Subregions_Thailand <- tibble(region = gcam.Thailand.parentregion, supplysector = A44.sector_en$supplysector) %>%
      # removing heating, as these nodes do not exist
      filter( !grepl( "heating", supplysector ) )
    ...
    X245.Satiation_flsp_bld_Subregions_Thailand <- satiation_flsp %>%
      tidyr::gather( gcam.consumer, value, comm ) %>%
    ...
    # We only want to keep degree days for region / consumer combinations that actually exist (i.e. Bangkok does not have resid rural)
    # To do this, make a table that contains region / consumer combinations where the share is nonzero
    keep_degree_days <- X245.SubregionalShares_bld_Subregions_Thailand %>%
      filter( subregional.population.share > 0 ) %>%
      select( c( region, gcam.consumer ) ) %>%
      distinct()
    ...
    X245.HDDCDD_constdds_SEA <- X245.HDDCDD_temp %>%
      ...
      # rename to target region(s)
      write_to_all_states(c( "region", "gcam.consumer", "nodeInput", "building.node.input", "thermal.building.service.input", "year", "degree.days" ),
                          region_list = gcam.Thailand.subregions ) %>%
      semi_join( keep_degree_days, by = c( "region", "gcam.consumer" ) )
    ...
    resid_subregions <- X245.DemandFunction_serv_bld_Subregions_Thailand %>%
    ...
    X245.GompFnParam_Thailand <- resid_subregions %>%
    ...
    if( length( zero_subregional_shares != 0 ) ) {
      ...
      X245.GompFnParam_Thailand <- X245.GompFnParam_Thailand %>%
        anti_join( zero_subregional_shares, by = c( "region", "gcam.consumer" ) )
